...
# clBGA is centered log BGA, zlBGA is z-scored BGA by year
# parallel prefixes for Chl
load(file='BGA+Chl_dark_centered_Z_2008-2021.Rdata')
dat0 = BGAdark # rename to fit the code below
print('Pigment variates',quote=F)
...
plot(dat0$dindex,dat0$zlBGA,type='l',col='blue',xlab='year',ylab='BGA, RFU')

# Quantiles & descriptive statistics
pvec = c(0.1,0.25,0.5,0.75,0.8,0.9,0.95)
print(quantile(dat0$zlBGA,probs=pvec))
print(c(mean(dat0$zlBGA),median(dat0$zlBGA),sd(dat0$zlBGA)))

# Density plot
dkern = density(dat0$zlBGA,kernel='epanechnikov',n=256)
windows()
...
THR = 0.85 # 90%ile is 0.855, 95%ile is 1.1277. Used 1 for Pareto. Note unit is s.d.
abline(h=THR,lty=2,lwd=2)

# Get indices of observations that exceed the threshold
gaplist = ReturnTimes(THR,dat0$dindex,dat0$zlBGA)
...
Xoriginal = dat0$zlBGA
Nshuf = length(Xoriginal)
ishuf = shuffle(Nshuf)
Xshuffle = Xoriginal[ishuf]

gapshuf = ReturnTimes(THR,dat0$dindex,Xshuffle)
...
plot(dat0$dindex,dat0$zlBGA,type='l',col='forestgreen',ylog='T',xlab='year',ylab='Precip, mm/d')
abline(h=THR,lty=2,lwd=2)
...
plot(dat0$doy,dat0$zlBGA,type='l',col='forestgreen',ylog='T',xlab='',ylab='Precip., mm/d')
abline(h=THR,lty=2,lwd=2)
#
...
plot(dat0$doy,dat0$zlBGA,type='l',col='forestgreen',ylog='T',xlab='',
ylab='Precip., mm/d')
abline(h=THR,lty=2,lwd=2)
#
plot(disp.mat[,1],disp.mat[,2],type='b',pch=19,lty=2,cex=1.5,lwd=2,col='blue',xlab='',
...
plot(disp.mat[,1],disp.mat[,2],type='b',pch=19,lty=2,cex=1.5,lwd=2,col='blue',xlab='',
     ylab='Extremes/Year')
grid()
text(2008,15,'A',cex=1.8)
plot(disp.mat[,1],disp.mat[,5],type='b',log='y',pch=19,lty=2,cex=1.5,lwd=2,col='blue',
     xlab='Year',ylab='Dispersion')
grid()
text(2008,150,'B',cex=1.8)
plot(xvals,yvals,type='l',lwd=2,col='red',ylim=c(0.005,1),xlim=c(0,8),
     log='y',xlab='Interval Between Events / Mean Interval',ylab='Rank / N')
points(rR,murP,type='p',pch=19,col='blue',cex=1.1,xlab='gap/mean')
# add shuffle to previous plot
points(rR.shuf,murP.shuf,type='p',pch=19,col='mediumorchid',cex=1.1)
points(xvals,yvals,type='l',lwd=2,col='red')
grid()
legend('bottomleft',legend=c('Poisson','Data','Shuffled Data'),
       lwd=c(3,NA,NA),pch=c(NA,19,19),col=c('red','blue','mediumorchid'),cex=1.5,bty='n')
...
grid()
text(2008,150,'B',cex=1.8)
plot(xvals,yvals,type='l',lwd=2,col='black',ylim=c(0.001,1),xlim=c(0,8),#log='y',
