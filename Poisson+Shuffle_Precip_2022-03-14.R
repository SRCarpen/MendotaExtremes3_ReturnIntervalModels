...
print('Water year matrices for PPT or P load',quote=F)
...
print('Summarize PPT data',quote=F)
print(summary(na.omit(PPT[,3])))

# Density plot
dkern = density(na.omit(PPT[,3]),kernel='epanechnikov',n=128)
windows()
...
THR = 20 # mm, 25 mm ~ 1 inch; 20 mm was fitted threshold for Pareto fit
abline(h=THR,lty=2,lwd=2)

# Get indices of observations that exceed the threshold
gaplist = ReturnTimes(THR,doy.dec,PPT[,3])
...
# Poisson cumulative ranks for original data ---------------------------------------------------
xrange = range(rR)
xvals = rR  #seq(xrange[1],xrange[2],length.out=30)
yvals = (exp(-xvals)) 
...
plot(doy.dec,PPT[,3],type='l',col='forestgreen',ylog='T',xlab='year',ylab='Precip, mm/d')
abline(h=THR,lty=2,lwd=2)
...
plot(doy.dec,PPT[,3],type='l',col='forestgreen',ylog='T',xlab='',ylab='Precip., mm/d')
abline(h=THR,lty=2,lwd=2)
#
...
plot(disp.mat[,1],disp.mat[,2],type='b',pch=19,lty=2,cex=1.5,lwd=2,col='blue',xlab='',
     ylab='Extremes/Year')
grid()
text(1940,19,'A',cex=1.8)
plot(disp.mat[,1],disp.mat[,5],type='b',log='y',pch=19,lty=2,cex=1.5,lwd=2,col='blue',
     xlab='Year',ylab='Dispersion')
grid()
text(1940,140,'B',cex=1.8)
plot(xvals,yvals,type='l',lwd=2,col='red',ylim=c(0.001,1),xlim=c(0,8),log='y',
...
# check correlations
cmat = as.data.frame(cbind(yvals,Pois.shuf,murP,murP.shuf))
...
print('2 sided K-S test, Poisson vs shuffled probs',quote=F)
ks1 = ks.test(yvals,murP.shuf,alternative='two.sided')
print(ks1)

print('2 sided K-S test, Poisson for shuffled rR vs shuffled probs',quote=F)
ks1shuf = ks.test(Pois.shuf,murP.shuf,alternative='two.sided')
print(ks1)
