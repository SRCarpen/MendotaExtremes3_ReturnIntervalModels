...
dat1 = PBYW2  # rename for compatibility with existing code
...
# Density plot
dkern = density(dat1$Totdis,kernel='epanechnikov',n=128)
windows()
...
plot(dat1$dindex.x,dat1$Totdis,type='l',col='black',ylog='T',xlab='year',ylab='P load')

print('Summarize Discharge data',quote=F)
pvec = c(0.5,0.75,0.8,0.9,0.95)
print(summary(dat1$Totdis))
print(quantile( dat1$Totdis,probs = pvec) )
...
THR = 218000 # m3/day, rounded-up 95 percentile for both streams combined
abline(h=THR,lty=2,lwd=2)

# Get indices of observations that exceed the threshold
nday = length(dat1$dindex.x)
DOY = dat1$dindex.x
gaplist = ReturnTimes(THR,DOY,dat1$Totdis)
...
Xoriginal = dat1$Totdis
Nshuf = length(Xoriginal)
ishuf = shuffle(Nshuf)
Xshuffle = Xoriginal[ishuf]

gapshuf = ReturnTimes(THR,DOY,Xshuffle)
...
plot(DOY,dat1$Totdis,type='l',col='forestgreen',ylog='T',xlab='year',ylab='Precip, mm/d')
abline(h=THR,lty=2,lwd=2)
...
plot(DOY,dat1$Totdis,type='l',col='forestgreen',ylog='T',xlab='',ylab='Precip., mm/d')
abline(h=THR,lty=2,lwd=2)
#
...
plot(disp.mat[,1],disp.mat[,2],type='b',pch=19,lty=2,cex=1.5,lwd=2,col='blue',xlab='',
     ylab='Extremes/Year')
grid()
text(1991,45,'A',cex=1.8)
plot(disp.mat[,1],disp.mat[,5],type='b',log='y',pch=19,lty=2,cex=1.5,lwd=2,col='blue',
     xlab='Year',ylab='Dispersion')
grid()
text(2019,180,'B',cex=1.8)
...
grid()
text(2019,180,'B',cex=1.8)
plot(xvals,yvals,type='l',lwd=2,col='red',ylim=c(0.001,1),xlim=c(0,9),#log='y',
